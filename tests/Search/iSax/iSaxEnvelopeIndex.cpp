...
TEST_CASE("iSaxIndex insert UTS envelope works") {
    ...
    SUBCASE("inserting first envelope works") {
        IndexEntry<Envelope> entry;
        entry.mts_summary = {{{-1.1, 0.1, -3.9}, {1.3, 2.3, 0.8}}};
        entry.subsequence_position = {13, 1};
    ...
    SUBCASE("inserting envelope with existing iSAX without splitting works") {
        ...
        entry.mts_summary = {{{-9.1, 10.3, -0.3}, {-3.8, 11.9, 0.6}}};
        entry.subsequence_position = {126, 9};
        index->insert(entry);
        auto isax_min = iSaxWord({0, 1, 0}, 1);
        const iSaxSplittableNode<Envelope> *node = index->get_first_layer_node({isax_min});
        REQUIRE(node != nullptr);
    ...
    SUBCASE("inserting envelope with new iSAX works") {
        ...
        entry.mts_summary = {{{4.2, 2.4, -5.7}, {8.8, 3.8, 5.3}}};
        entry.subsequence_position = {352, 111};
        index->insert(entry);
    ...
    SUBCASE("inserting envelope with existing iSAX with splitting works") {
        ...
        entry.mts_summary = {{{-4.1, 4.5, -1.6}, {-1.8, 6.9, -0.6}}};
        entry.subsequence_position = {78, 91};
        index->insert(entry);
    ...
    SUBCASE("inserting envelope into non-first-layer node without splitting works") {
        ...
        entry.mts_summary = {{{-0.6, 1.3, -10.6}, {1.8, 3.1, -5.6}}};
        entry.subsequence_position = {555, 555};
        index->insert(entry);
    ...
    SUBCASE("inserting envelope into non-first-layer node with one extra split works") {
        IndexEntry<Envelope> entry;
        entry.mts_summary = {{{-3.1, 0.1, -3.9}, {1.3, 2.3, 0.8}}};
        entry.subsequence_position = {13, 1};
        index->insert(entry);
        entry.mts_summary = {{{-9.1, 10.3, -0.3}, {-3.8, 11.9, 0.6}}};
        entry.subsequence_position = {126, 9};
        index->insert(entry);
        // Triggers first split
        entry.mts_summary = {{{-1.6, 5.3, -10.6}, {1.8, 3.1, -5.6}}};
        entry.subsequence_position = {555, 555};
        index->insert(entry);
        ...
        entry.mts_summary = {{{-4.1, 1.5, -1.6}, {-1.8, 6.9, -0.6}}};
        entry.subsequence_position = {78, 91};
        index->insert(entry);
    ...
    SUBCASE("inserting envelope successfully triggers two splits") {
        ...
        entry.mts_summary = {{{-2.6, 5.3, -10.6}, {1.8, 3.1, -5.6}}};
        entry.subsequence_position = {555, 555};
        index->insert(entry);
    ...
    SUBCASE("inserting the same envelope multiple times triggers splits until max resolution is reached") {
        ...
        IndexEntry<Envelope> entry;
        entry.mts_summary = envelope;
        entry.subsequence_position = {100, 300};
        index->insert(entry);
        entry.mts_summary = envelope;
        entry.subsequence_position = {200, 200};
        index->insert(entry);
        entry.mts_summary = envelope;
        entry.subsequence_position = {300, 100};
        index->insert(entry);
...
TEST_CASE("iSaxIndex insert MTS envelope works") {
    ...
    SUBCASE("inserting one envelope works") {
        IndexEntry<Envelope> entry;
        entry.mts_summary = {{{0.2, -5.5}, {1.1, -3.1}}, {{-1.9, 2.7}, {-0.6, 3.8}}, {{1.9, 2.7}, {3.1, 5.7}}};
        entry.subsequence_position = {64, 37};
        index->insert(entry);
    ...
    SUBCASE("inserting multiple envelopes works") {
        ...
        entry.subsequence_position = {64, 37};
        index->insert(entry);
        entry.mts_summary = {{{0.1, -0.5}, {0.8, 1.3}}, {{-1.3, 1.7}, {0.6, 2.3}}, {{1.3, 1.7}, {2.3, 3.7}}};
        entry.subsequence_position = {128, 81};
        index->insert(entry);
        entry.mts_summary = {{{0.5, -0.3}, {0.6, 1.1}}, {{-1.1, 1.3}, {0.3, 1.7}}, {{1.1, 1.3}, {2.1, 3.3}}};
        entry.subsequence_position = {256, 19};
        index->insert(entry);
