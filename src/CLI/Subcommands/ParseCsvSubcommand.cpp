...
#include "Util/Constants/ErrorMessages.hpp"
...
ParseCsvSubcommand::ParseCsvSubcommand(CLI::App &app) {
    ...
    auto &non_negative_real = validators.get_non_negative_real_validator();
    ...
    csv_subcommand->add_option("-d,--dataset", m_dataset_path, "Output dataset path relative to `DATA`")->required();
    csv_subcommand->add_option("-n,--num_series", m_num_series, "Max number of series")
        ->required()
        ->check(positive_int);
    ...
    csv_subcommand->add_option("-m,--series_len", m_series_len, "Length of series")->required()->check(positive_int);
}

void ParseCsvSubcommand::validate_arguments() {
    if (m_l_min > m_l_max) throw std::runtime_error(L_MIN_GT_L_MAX);
}

void ParseCsvSubcommand::execute(const CommonOptions &common_opts) {
    // Set up run setting properties
    create_dataset_from_csv(m_csv_paths, m_num_series, m_l_min, m_l_max, ',', m_min_subs_sd, common_opts.m_seed);
}
