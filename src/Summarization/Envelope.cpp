...
vec<Envelope> ulisse_envelope_raw(const vec<Real>& ts, const UlisseEnvelopeParams& env_params) {
    ...
    vec<Envelope> envelopes(num_env, {vec<Real>(segments_per_env, INF), vec<Real>(segments_per_env, -INF)});
...
vec<IndexEntry<Envelope>> EnvelopeEntryGenerator::get_entries(const vec<vec<Real>>& mts, uint series_ind) {
    ...
    for (MtsNumChannelsT c = 0; c < m_num_channels; ++c) {
        ...
        for (size_t i = 0; i < channel_envs.size(); ++i) {
            ...
            if (c == 0) {
                entries[i].subsequence_info = {series_ind, start_pos, length};
