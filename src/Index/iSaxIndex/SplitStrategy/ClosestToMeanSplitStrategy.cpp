...
template <typename T>
ClosestToMeanSplitStrategy<T>::ClosestToMeanSplitStrategy(Real max_std_dist) : m_max_std_dist(max_std_dist) {}

template <typename T>
SaxSplitIndex ClosestToMeanSplitStrategy<T>::get_split_ind(const iSaxSplittableLeaf<T> *leaf,
    ...
    for (MtsNumChannelsT c = 0; c < num_channels; ++c) {
        ...
        for (SaxSegIndT s = 0; s < num_segments; ++s) {
            ...
            if ((*mid_breakpoint - mu) / sigma <= m_max_std_dist) {
                Real diff = std::abs(*mid_breakpoint - mu);
                if (diff < min_diff) {
                    split_ind = {s, c};
                    min_diff = diff;
                }
...
DECLARE_ENTRY_DATA_SPECS(ClosestToMeanSplitStrategy)
