...
void DatasetStatsLogger::write_entry(const str &dataset_file, uint ts_ind, MtsNumChannelsT channel,
    ...
    instance.write_row(dataset_stats_path,
                       {{DSTC::DATASET_FILE, dataset_file},
                        ...
                        {DSTC::TOTAL_VAR_STDS, instance.get_collection_str(stats.m_total_var_stds)},
                        {DSTC::AUTOCORR_MEANS, instance.get_collection_str(stats.m_autocorr_means)},
                        {DSTC::AUTOCORR_STDS, instance.get_collection_str(stats.m_autocorr_stds)}},
